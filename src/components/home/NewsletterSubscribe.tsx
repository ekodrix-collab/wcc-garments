...
export default function NewsletterSubscribe() {
  ...
  return (
    <section className="relative w-full bg-[var(--bg-surface)] text-[var(--text)] py-20 px-6 flex items-center justify-center overflow-hidden transition-colors duration-500">
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto text-center">
        {/* Eyebrow label */}
        <p
          className="text-xs tracking-[0.25em] text-gold uppercase mb-10 font-bold"
        ...
        {status !== "success" && (
          <p
            className="text-[1.35rem] leading-relaxed text-[var(--text)] font-light mb-12 max-w-xl mx-auto"
            style={{ fontFamily: "'Cormorant Garamond', 'Garamond', Georgia, serif" }}
          >
        ...
        {status === "success" ? (
          ...
          <>
            {/* Input + Button */}
            <div className="flex items-stretch w-full max-w-lg mx-auto border border-[var(--border)] bg-transparent">
              <input
                type="email"
                value={email}
                onChange={(e) => {
                ...
                className="flex-1 bg-transparent px-5 py-4 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] outline-none tracking-wide"
                style={{ fontFamily: "'Cormorant Garamond', 'Garamond', Georgia, serif", fontSize: "1rem" }}
              />
              <button
                onClick={handleSubscribe}
                disabled={status === "loading"}
                className="relative bg-gold text-white px-8 py-4 text-xs tracking-[0.22em] uppercase font-light transition-all duration-300 hover:bg-gold-dark active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed overflow-hidden group"
                style={{ fontFamily: "'Cormorant Garamond', 'Garamond', Georgia, serif" }}
              >
                {status === "loading" ? (
